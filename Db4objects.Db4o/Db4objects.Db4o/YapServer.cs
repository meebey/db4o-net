...
{
	internal class YapServer : Db4objects.Db4o.IObjectServer, Db4objects.Db4o.Ext.IExtObjectServer
		, Sharpen.Lang.IRunnable, Db4objects.Db4o.Foundation.Network.IYapSocketFakeServer
	{
		...
		internal YapServer(Db4objects.Db4o.YapFile a_yapFile, int a_port)
		{
		...
		public virtual bool Close()
		{
			lock (Db4objects.Db4o.Db4o.Lock)
			{
				...
				lock (i_threads)
				{
					...
					while (i.MoveNext())
					{
						((Db4objects.Db4o.YapServerThread)i.Current).Close();
					}
				}
				i_yapFile = null;
				return isClosed;
		...
		internal virtual Db4objects.Db4o.YapServerThread FindThread(int a_threadID)
		{
			lock (i_threads)
			{
				...
				while (i.MoveNext())
				{
					Db4objects.Db4o.YapServerThread serverThread = (Db4objects.Db4o.YapServerThread)i
						.Current;
			 ...
			 config)
		{
			try
			{
				Db4objects.Db4o.YapClient client = new Db4objects.Db4o.YapClient(config, OpenClientSocket
			...
			)
		{
			...
			try
			{
				Db4objects.Db4o.YapServerThread thread = new Db4objects.Db4o.YapServerThread(this
					, i_yapFile, serverFake, i_threadIDGen++, true);
				lock (i_threads)
				{
		...
		internal virtual void RemoveThread(Db4objects.Db4o.YapServerThread aThread)
		{
			lock (i_threads)
			{
		...
		public virtual void Run()
		{
			...
			while (i_serverSocket != null)
			{
				...
				{
					Db4objects.Db4o.YapServerThread thread = new Db4objects.Db4o.YapServerThread(this
						, i_yapFile, i_serverSocket.Accept(), i_threadIDGen++, false);
					lock (i_threads)
					{
						i_threads.Add(thread);
					}
