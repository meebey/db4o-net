...
{
	internal sealed class YShort : Db4objects.Db4o.YapJavaClass
	{
		internal const int LENGTH = Db4objects.Db4o.YapConst.SHORT_BYTES + Db4objects.Db4o.YapConst
			.ADDED_LENGTH;

		private static readonly short i_primitive = (short)0;

		public YShort(Db4objects.Db4o.YapStream stream) : base(stream)
		{
		}
		...
		internal static short ReadShort(Db4objects.Db4o.YapReader a_bytes)
		{
			int ret = 0;
			for (int i = 0; i < Db4objects.Db4o.YapConst.SHORT_BYTES; i++)
			{
		...
		internal static void WriteShort(int a_short, Db4objects.Db4o.YapReader a_bytes)
		{
			for (int i = 0; i < Db4objects.Db4o.YapConst.SHORT_BYTES; i++)
			{
				a_bytes._buffer[a_bytes._offset++] = (byte)(a_short >> ((Db4objects.Db4o.YapConst
					.SHORT_BYTES - 1 - i) * 8));
			}
		}
